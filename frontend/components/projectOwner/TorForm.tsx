  ...
  return (
    <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div className={styles.grid}>
          ...
          <label className={styles.full}>
            ...
            <input required value={form.projectName} 
              ...
              placeholder="เช่น โครงการพัฒนาระบบบริหารจัดการเอกสาร" 
            />
          </label>
          ...
          <label>
            หน่วยงาน 
            <b>*</b>
            <input required value={form.agencyName} 
              onChange={(e) => setForm(
                { ...form, agencyName: e.target.value }
              )} 
              placeholder="ชื่อหน่วยงานเจ้าของโครงการ" 
            />
          </label>
          ...
          <label>
            ...
            <input 
              ...
              value={form.budget ?? ""} 
              onChange={(e) => setForm(
                { ...form, budget: e.target.value === "" ? null : Number(e.target.value) }
              )} 
            />
          </label>

          <label>
            วันสิ้นสุดรับข้อเสนอ
            <input 
              type="date" 
              value={form.submissionDeadline} 
              onChange={(e) => setForm(
                { ...form, submissionDeadline: e.target.value }
              )} 
            />
          </label>
          ...
          <label 
            className={styles.full}>
              ...
              <textarea 
                ...
                placeholder="อธิบายภาพรวมและที่มาของโครงการ" 
              />
          </label>
      ...
      {(["objectives", "scopeOfWork"] as const).map((field, fieldIndex) => (
        <FormSection 
          key={field} 
          number={`0${fieldIndex + 2}`} 
          title={field === "objectives" ? "วัตถุประสงค์" : "ขอบเขตงาน"} 
          subtitle="เพิ่มรายละเอียดได้มากกว่าหนึ่งรายการ">
              ...
              {form[field].map((item, index) => (
                ...
                >
                  ...
                  <button 
                    ...
                    aria-label="ลบรายการ" 
                    onClick={() => setForm(
                      (current) => ({ ...current, [field]: current[field].filter((_, i) => i !== index) })
                    )}>
                      <Trash2 size={17} />
                  </button>
      ...
      <FormSection 
        ...
        subtitle="กำหนดน้ำหนักและเงื่อนไขที่จำเป็น"
      >
        <div className={styles.list}>
          {form.requirements.map((requirement, index) => (
            ...
            >
              <input 
                ...
                onChange={(e) => setForm((current) => ({ ...current, requirements: current.requirements.map((item, i) => i === index ? { ...item, description: e.target.value } : item) }))} 
                placeholder="ระบุคุณสมบัติหรือเงื่อนไข" 
              />
              ...
              <label>
                ...
                <input 
                  ...
                  onChange={(e) => setForm((current) => ({ ...current, requirements: current.requirements.map((item, i) => i === index ? { ...item, weight: Number(e.target.value) } : item) }))} 
                />
              </label>
              ...
              <label 
                className={styles.checkbox}>
                  <input 
                    ...
                    onChange={(e) => setForm((current) => ({ ...current, requirements: current.requirements.map((item, i) => i === index ? { ...item, mandatory: e.target.checked } : item) }))} 
                  /> 
                  จำเป็นต้องผ่าน
              </label>
              ...
              <button 
                ...
                onClick={() => setForm((current) => ({ ...current, requirements: current.requirements.filter((_, i) => i !== index) }))}
              >
                <Trash2 size={17} />
              </button>
          ...
          <button 
            className={styles.addButton} 
            type="button" onClick={() => setForm((current) => ({ ...current, requirements: [...current.requirements, { description: "", weight: 0, mandatory: false }] }))}
          >
            <Plus size={16} /> 
            เพิ่มเงื่อนไข
          </button>
      ...
      <FormSection 
        ...
        subtitle="ข้อมูลสำหรับติดต่อเกี่ยวกับโครงการ"
      >
        <div className={styles.grid}>
          <label>
            ชื่อผู้ติดต่อ
      ...
      <div className={styles.actions}>
        <span>ข้อมูลจะถูกจัดเก็บเป็นฉบับร่าง</span>
        <button 
